    ...
    tasks: Task[] = [
        {
            ...
            remember: `
                ...
                Если в такую ячейку заходили, или вне поля, или 0, не пополняем очередь этой координатой. Как только поиск закончился, пополняем колво осторовов и  проходим по следующим буквам. Кусочки острова 1 меняем на 0.
        ...
        },
        {
            ...
            `,
            remember: `
            Topological Sort - сортировка вершин в завимимости от очередности и связки. в основном используется для планирования заданий на основе заданных зависимостей между заданиями. Для этого нужно adj list и проверки, что мы не в цикле.
            DFS - Проходимся по adj list, отправляем в dfs. Visited  и Cycle sets. В dfs поверяям, есть ли наш узел в цикле, значит, некорретный граф.  Если в visited - ок. Елси нет, добавляем в Cycle, что значит мы сейчас проходим цепочку с определенными элементам. Проходимся по всем узлам, проверяя, что вернет (fale -> return false). Если прошли цикл и все ок, удаляем текущий элемент из цикла, добавляем в visited и в результат. Завершаем true.
